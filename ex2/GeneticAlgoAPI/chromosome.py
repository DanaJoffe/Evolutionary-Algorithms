...
class VectorChromosome(Chromosome, ABC):
    """ a chromosome that can be accessed as a vector """
    @abstractmethod
    def __getitem__(self, k):
        """  """
        raise NotImplemented
...
class ListChromosomeBase(VectorChromosome, ABC):
...
class IntChromosome(VectorChromosome):
    def __init__(self, length, genome=None):
        self.length = length
        self.genome = genome
        # initialize random bit list
